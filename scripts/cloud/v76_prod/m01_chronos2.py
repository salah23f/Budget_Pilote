"""m01_chronos2.py — Amazon Chronos-Bolt zero-shot forecasting (standalone)."""
import time
import modal

app = modal.App("flyeas-v76-chronos2")
volume = modal.Volume.from_name("flyeas-v75", create_if_missing=True)

image = modal.Image.debian_slim(python_version="3.11").pip_install(
    "numpy==1.26.4", "pandas==2.2.2", "pyarrow==16.1.0",
    "torch==2.3.1", "chronos-forecasting>=1.5.1", "huggingface_hub>=0.24",
)


@app.function(image=image, gpu="A10G", volumes={"/vol": volume}, timeout=60 * 60)
def run(context_len: int = 64, horizon: int = 1, chunk: int = 256):
    import os
    import numpy as np
    import pandas as pd
    import torch
    ...
    for r in df["route"].unique():
        ...
        if len(sub) < context_len + 1:
            continue
        p = sub["price_usd"].values[-context_len - horizon:]
        pairs.append((r, p[:-horizon], float(p[-1])))
    print(f"[chronos2] {len(pairs):,} routes, chunk={chunk}")

    rows = []
    for i in range(0, len(pairs), chunk):
        batch = pairs[i:i + chunk]
        ctx = [torch.tensor(x[1]).float() for x in batch]
        try:
            ...
            for j, (r, _, actual) in enumerate(batch):
                rows.append({"route": r, "actual": actual,
                             "prediction": float(q[j, 0, 1]),
                             "q10": float(q[j, 0, 0]), "q90": float(q[j, 0, 2])})
